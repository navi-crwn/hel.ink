<x-app-layout>
    <x-slot name="header">
        <x-page-header title="Manage Tag" subtitle="Curate every link labeled {{ $tag->name }}">
            <x-slot name="actions">
                <a href="{{ route('tags.index') }}" class="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">Back to tags</a>
        ...
        <div class="mx-auto max-w-6xl space-y-6 sm:px-6 lg:px-8">
            ...
            <div class="grid gap-6 md:grid-cols-2">
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <h2 class="mt-2 text-2xl font-semibold text-slate-900 dark:text-white">{{ $tag->name }}</h2>
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Created {{ optional($tag->created_at)->diffForHumans() }} · {{ $links->total() }} links</p>
                    <form method="POST" action="{{ route('tags.update', $tag) }}" class="mt-4 space-y-3">
                        @csrf
                        @method('PUT')
                        <label class="text-sm font-medium text-slate-700 dark:text-slate-200" for="tag-name">Rename tag</label>
                        <input id="tag-name" type="text" name="name" value="{{ $tag->name }}" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white">
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <div class="mt-4 space-y-3 text-sm">
                        <a href="{{ route('dashboard', ['tag' => $tag->id]) }}" class="flex items-center justify-between rounded-2xl border border-slate-200 px-4 py-3 text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
                            <span>View in main dashboard</span>
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                            </svg>
                        </a>
                        <a href="{{ route('tags.index') }}#tag-{{ $tag->id }}" class="flex items-center justify-between rounded-2xl border border-slate-200 px-4 py-3 text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
                            <span>Scroll to card</span>
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                            </svg>
                        </a>
                        <form method="POST" action="{{ route('tags.destroy', $tag) }}" onsubmit="return confirm('Delete this tag?')" class="rounded-2xl border border-rose-200 px-4 py-3 text-rose-600 dark:border-rose-900/30 dark:text-rose-200">
                            @csrf
                            @method('DELETE')
                            ...
                            <p class="mt-1 text-xs text-rose-500 dark:text-rose-200/80">Links akan kehilangan tag ini.</p>
                        </form>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                        <h3 class="text-lg font-semibold text-slate-900 dark:text-white">Links using {{ $tag->name }}</h3>
                        <p class="text-sm text-slate-500 dark:text-slate-400">Filter and manage every link that carries this tag.</p>
                    </div>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            @forelse ($links as $link)
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-900/40">
                                    ...
                                    <td class="py-3 align-top text-slate-600 dark:text-slate-300">{{ $link->folder->name ?? '—' }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="py-6 text-center text-slate-500 dark:text-slate-400">No links with this tag yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
