    ...
    <body class="font-sans antialiased bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100">
        @auth
            <div x-data="{ sidebarOpen: window.innerWidth >= 1024 }" 
                 x-init="
                    // Monitor resize - auto show/hide sidebar
                    window.addEventListener('resize', () => {
                        if (window.innerWidth < 1024 && sidebarOpen) {
                            // Layar mengecil -> hide sidebar
                            sidebarOpen = false;
                        } else if (window.innerWidth >= 1024 && !sidebarOpen) {
                            // Layar membesar -> show sidebar
                            sidebarOpen = true;
                        }
                    });
                 "
                ...
                <div class="min-h-screen"
                     :style="'margin-left: ' + (sidebarOpen && window.innerWidth >= 1024 ? '256px' : '0px') + '; transition: margin-left 0.35s cubic-bezier(0.4, 0, 0.2, 1);'">
